...
浏览器打开 [http://localhost:3000](http://localhost:3000)。首页可以填写选题并预览 8 份 Markdown；“历史项目”页面读取本地 `output` 目录；项目详情页支持 Markdown 渲染、单文件导出、生成修改版和扫描本地素材文件夹。在“07_视觉参考提示词”中还可以选择常用比例并生成封面图片。旧项目中的 `06_视觉参考提示词.md` 仍可继续使用封面生成器。项目详情页的“镜头执行”还支持逐镜头记录计划/实际时长、删镜、补拍、现场问题和新增镜头；保存复盘后可生成独立的下一版脚本、分镜、拍摄清单和成片执行稿，确认应用时会保留旧镜头的拍摄状态、素材判断与备注。
...
- 增加拍摄复盘：计划/实际对照、现场问题、补拍记录与独立 revision 包
- 复盘修订包应用时自动同步脚本、分镜、拍摄清单、成片执行稿，并合并保留镜头现场状态
- 生成与修改提示词加入具体场景、口语化和反模板表达约束，质量校验会拦截明显 AI 味表达
