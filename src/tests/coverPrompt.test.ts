...
test("封面提示词重生成要求从内容提炼视觉而不是复用固定海报", () => {
  const prompt = buildCoverPromptRegenerationPrompt("AI 为什么替代不了人类", "主标题：AI不用负责，所以替代不了你。核心判断：需要承担责任的工作，AI只能辅助。", "3:4");
  assert.match(prompt, /核心判断、目标读者与最有传播力的冲突/u);
  assert.match(prompt, /不得生成任何文字/u);
  assert.match(prompt, /不沿用内容中的固定配色/u);
  assert.match(prompt, /项目选题/u);
  assert.match(prompt, /文案内容/u);
