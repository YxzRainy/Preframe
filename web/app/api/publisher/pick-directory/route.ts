...
/** 使用系统原生文件夹选择器选择一个目录，仅返回路径，不设置任何状态。 */
  ...
  try {
    ...
    const result = await new Promise<string | null>((resolve) => {
      const child = spawn("osascript", [
        ...
        "-e",
        'tell application "System Events" to choose folder with prompt "选择成片监听目录"',
        "-e",
    ...
    return apiError(error, "publisher", "文件夹选择失败。", 500);
  }
}
